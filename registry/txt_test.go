...
// TestUserTXTRecordsNotParsedForOwnership tests that invalid user TXT records don't cause parsing errors
func TestUserTXTRecordsNotParsedForOwnership(t *testing.T) {
	p := inmemory.NewInMemoryProvider()
	ctxt := context.Background()

	// Create TXT registry with "extdns" prefix
	r, _ := NewTXTRegistry(p, "extdns-", "", "owner-1", time.Hour, "", []string{}, []string{}, false, nil)

	// Add a user TXT record with content that would fail ownership parsing
	// This simulates the actual bug - user TXT records with arbitrary content
	userTXTRecord := &endpoint.Endpoint{
		DNSName:    "peer1.test-zone.example.org",
		RecordType: endpoint.RecordTypeTXT,
		Targets:    endpoint.Targets{"v=1;id=enode1;addr=/ip4/10.0.0.1/tcp/30303/p2p/Qm..."},
	}

	// Add an actual registry TXT record for a different endpoint
	registryTXTRecord := &endpoint.Endpoint{
		DNSName:    "extdns-peer2.test-zone.example.org",
		RecordType: endpoint.RecordTypeTXT,
		Targets:    endpoint.Targets{"heritage=external-dns,external-dns/owner=owner-1,external-dns/resource=test"},
	}

	// Add both records to the provider
	p.CreateZone(testZone)
	p.ApplyChanges(ctxt, &plan.Changes{
		Create: []*endpoint.Endpoint{userTXTRecord, registryTXTRecord},
	})

	// Get records from registry - this should not fail due to parsing user TXT content
	records, err := r.Records(ctxt)
	require.NoError(t, err, "Registry should handle user TXT records without parsing errors")

	// Should contain the user TXT record as-is
	var foundUserRecord *endpoint.Endpoint
	for _, record := range records {
		if record.DNSName == "peer1.test-zone.example.org" && record.RecordType == endpoint.RecordTypeTXT {
			foundUserRecord = record
			break
		}
	}

	require.NotNil(t, foundUserRecord, "User TXT record should be present in results")
	assert.Equal(t, endpoint.Targets{"v=1;id=enode1;addr=/ip4/10.0.0.1/tcp/30303/p2p/Qm..."}, foundUserRecord.Targets)

	// User TXT record should not have any owner labels since no registry record exists for it
	ownerID, hasOwner := foundUserRecord.Labels[endpoint.OwnerLabelKey]
	assert.False(t, hasOwner, "User TXT record should not have owner labels when no registry record exists")
	assert.Equal(t, "", ownerID, "Owner ID should be empty for unregistered TXT record")

	// Should NOT contain the registry TXT record in the results (it's internal to the registry)
	for _, record := range records {
		assert.NotEqual(t, "extdns-peer2.test-zone.example.org", record.DNSName, "Registry TXT record should not appear in endpoint results")
	}
}
